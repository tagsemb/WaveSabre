            ...
            while (reader.Read() && !(reader.NodeType == XmlNodeType.EndElement && reader.Name == "TrackSendHolder"))
            {
                if (reader.NodeType == XmlNodeType.Element)
                {
                    switch (reader.Name)
                    {
                        ...
                        case "AutomationTarget": break; // ignore
                        case "Max": break; // ignore
                        case "MidiControllerRange": break; // ignore
                        case "ModulationTarget": break; // ignore
                        case "LockEnvelope": break; // ignore
                        case "LomId": break; // ignore
                        case "Send": break; // ignore

                        default:
                            throw new Exception("parseTrackSendHolder got unexpected reader.Name " + reader.Name);
                    }
                }
            }
        }
